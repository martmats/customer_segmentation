...
if openai_api_key:
    ...
    st.sidebar.success("API Key ingresada correctamente.")  # Confirm API key is successfully entered
else:
    st.warning("Por favor, ingresa tu API Key de OpenAI para comenzar.")
    st.stop()  # Stop the app until API key is provided
        ...
        if selected_features:
            ...
            if st.sidebar.button("Identificar Perfiles"):
                ...
                if st.button("Generar Recomendaciones de Marketing"):
                    st.write("Generando recomendaciones...")  # Debugging: Show that the button has been pressed
                ...
                if st.button("Obtener Respuesta a la Pregunta"):
                    if question:
                        st.write(f"Pregunta recibida: {question}")  # Debugging: Acknowledge question
                        ...
                        st.write(f"Resumen de los datos: {data_summary}")  # Debugging: Show data summary
